...
public class GameBoard extends GridPane {
  ...
  /**
   * Set the listener to handle an event when mouse is right-Clicked
   *
   * @param listener listener to add
   */
  ...
  /**
   * Triggered when mouse is right-clicked. Call the attached listener.
   *
   * @param mouseEvent mouse event
   */
  public void rightClicked(MouseEvent mouseEvent) {
    if (mouseEvent.getButton() == MouseButton.SECONDARY && rightClickedListener != null) {
      rightClickedListener.setOnRightClicked(this);
    }
  }

  /**
   * fadeout the set of blocks.
   *
   * @param gameBlockCoordinates set of game blocks
   */
  public void clear(Set<int[]> gameBlockCoordinates) {
    for (int[] a : gameBlockCoordinates) {
      getBlock(a[0], a[1]).fadeOut();
    }
  }
}
